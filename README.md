...
GuideCs has no functionality on its own and requires the base GUIDE addon to be installed and running. This wrapper simply adds C# typing and function calls. It does not cover all objects, variables, or functions and is limited to those designated in the original addon as public or ambiguous.

A wrapper works by letting the original addon do all the work normally with no knowledge or understanding of the C# wrapper while the wrapper simply makes calls back to the addon with as much assumed type-safety as can be mustered between the two languages. Functionality wise, it works almost identically to standard GUIDE but with an extra interop layer. I recommend doing a little learning about wrappers before picking this up if the concept is new to you.
...
This was made to support my own project so updates will be minimal. If you find a bug, submit a bug ticket. Otherwise GL HF!<br><br><br>
